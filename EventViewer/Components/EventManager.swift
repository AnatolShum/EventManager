...
public final class EventManager: NSPersistentContainer {
    ...
    public func fetchEvents() -> [Event] {
        ...
        do {
            ...
            return events.map { event in
                let parameters = event.parameters?.reduce(into: ParameterSet(), { (result, dbParameter) in
                    let key = dbParameter.key
                    if let value = dbParameter.stringValue {
                        result[key] = .string(value)
                    } else if let value = dbParameter.integerValue {
                        result[key] = .integer(Int(value))
                    }
                })
                return Event(id: event.id, parameters: parameters!, createdAt: event.createdAt!)
            }
        } catch {
    ...
    public func deleteEvent(event: Event) {
        ...
        do {
            let events = try viewContext.fetch(request)
            if let eventToDelete = events.first(where: { $0.id == event.id && $0.createdAt == event.createdAt }) {
                print(eventToDelete)
